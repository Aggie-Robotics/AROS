...
impl Vision {
    ...
    pub fn set_signature(&self, index: SignatureIndex, signature: Signature) {
        unsafe { vexDeviceVisionSignatureSet(self.port.device(), &mut (index, signature).into()) }
    }

    pub fn auto_white_balance(&self, enable: bool) {
    ...
    pub fn set_white_balance<T>(&self, white_balance: RGB<T>) where T: Into<u8> {
    ...
    pub fn set_led<T>(&self, color: RGB<T>) where T: Into<u8> {
    ...
    pub fn set_led_brightness(&self, percent: Percent) {
